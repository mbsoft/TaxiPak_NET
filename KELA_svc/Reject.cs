...
namespace SUTI_svc
{
	...
	public class Reject
	{
		private static readonly ILog log = LogManager.GetLogger(typeof(Reject));
		...
		public Reject(XmlNode confirmNode, int msgCounter, XmlDocument xDoc)
		{
			...
			if ( sOrderID.Length > 0 )
				ConfirmOrderReject( sOrderID, sVehicleID );

		}


		private void ConfirmOrderReject( string sOrderID, string sVehicleID )
		{
			...
			{
				ct.CommandText = String.Format("update ilink set status='REJECTED' where tpak_nbr={0}",
					this.sOrderID);
				ct.CommandType = CommandType.Text;
				ct.ExecuteNonQuery();

				ct.CommandText = String.Format("insert into callh values (0,{0}, 0, 'H', 'T',{1},'{2}','{3}','LTX-REJ',0,0,0,' ',' ','{4}',{5},0,0,0)",
		...
		public string ReplyReject()
		{
			...
			w.WriteAttributeString("msgType", "2003");
			w.WriteAttributeString("msgName", "Order Reject Confirmation");
